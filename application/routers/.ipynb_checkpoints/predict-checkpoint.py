...
def predict(request: Request, params: predictParams = Depends()):
    '''
    主要功能：提供模型檔與圖檔路徑，進行模型predict，可適用於binay、multiclass model。
    ...
    * Request body (class_dict): 若為 multiclass 需填入，若為 binary 不會引入此參數可忽略。
    ...
    logger.info(f'[predict] request_ip={client_host_ip} ')
    ...
    try:
        result=dict()
        with predictor(model_path,gpu,image_path,class_dict,binary_thres,False,None,True,excel_target) as predict:
        ...
        logger.warning(f'[predict] {str(e)} ')
        return {
        'status': 'error',
        'message': str(e)
        }
